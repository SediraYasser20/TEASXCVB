    ...
    if ($mo->fetch($mo_id) <= 0) {
        ...
        if (!$error) {
            $result = $object->addline(
                ...
                $date_end,          // Date end
                'mo',               // Product type (custom type for MO)
...
$arrayforbutaction[] = array(
    'lang' => 'bills',
    'enabled' => (isModEnabled('invoice') && $object->statut > Commande::STATUS_DRAFT && !$object->billed && $object->total_ttc >= 0 && (!isModEnabled('shipping') || $object->countNbOfShipments() > 0)),
    ...
    'url' => '/compta/facture/card.php?action=create&token=' . newToken() . '&origin=' . urlencode($object->element) . '&originid=' . $object->id . '&socid=' . $object->socid
        ...
        // Delete order (Restricted to admin)
        if ($usercandelete && $user->admin) {
            if ($numshipping == 0) {

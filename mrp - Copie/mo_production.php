    ...
    if ($resql) {
        ...
        if ($obj && $obj->pmp > 0) {
            $pricetoprocess = $obj->pmp;
					...
					if ($qtytoprocess != 0) {
						...
						if (GETPOSTISSET('idwarehousetoproduce-'.$line->id.'-'.$i)) {	// If there is a warehouse to set
							...
							if (isModEnabled('productbatch') && $tmpproduct->status_batch && (!GETPOST('batchtoproduce-'.$line->id.'-'.$i))) {
								if ($line->fk_product != 31 || $total_quantity_for_line == 1) {
									 $langs->load("errors");
									 setEventMessages($langs->trans("ErrorFieldRequiredForProduct", $langs->transnoentitiesnoconv("Batch"), $tmpproduct->ref), null, 'errors');
									 $error++;
								}
							...
							if ($line->fk_product == 31 && $total_quantity_for_line > 1) {
								if ($qtytoprocess > 0) { // Ensure there's a quantity for this specific form input
									for ($unit_count = 0; $unit_count < $qtytoprocess; $unit_count++) {
										$current_batch_to_use_unit = '';
										if ($is_category_24_product_server) {
											$current_batch_to_use_unit = $object->ref;
										} else {
											$current_batch_to_use_unit = $object->ref . '-' . $serial_counter_for_product_31;
										}
										...
										// Price in MoLine for a single unit would be $unit_price.
										// Depending on how MoLine stores price (total or unit), this might need adjustment.
										// Assuming MoLine expects total price for its quantity (which is 1 here).
										// $moline_unit->price = $unit_price; // If MoLine has a price field. Standard Dolibarr MoLine does not directly. Cost is implied by stock move.
										...
										$pos++;
										$serial_counter_for_product_31++;
									}
									if ($error) break; // Break from while GETPOSTISSET loop if error occurred in for loop
								}
							} else { // Not product 31, OR product 31 with total_quantity_for_line == 1
								if ($is_category_24_product_server) {
									$current_batch_to_use = $object->ref;
								} elseif ($line->fk_product == 31) { // Implies total_quantity_for_line is 1 due to outer if condition
									$current_batch_to_use = $object->ref;
								} else {
								...
								if (!$error) {
									...
									$moline->qty = $qtytoprocess;
									$moline->batch = $current_batch_to_use; // Batch used for the whole quantity
					...
					if ($error && $line->fk_product == 31 && $total_quantity_for_line > 1) { // If error occurred in product 31 multi-qty processing, break from while
						break; 
					}
					$i++;
				}
...
print '</td>';
					...
					if (in_array($action, array('consumeorproduce', 'consumeandproduceall'))) {
						...
						$maxQty = 1;
						print '<tr data-product-id="'.(!empty($line->fk_product) ? $line->fk_product : '0').'" data-mo-ref="'.dol_escape_htmltag($object->ref).'" data-max-qty="'.$maxQty.'" name="batch_'.$line->id.'_'.$i.'">';
						// Product
        ...
        print '</td>';
						...
						if (isModEnabled('productbatch')) {
							...
							if ($tmpproduct->status_batch) {
								...
								// $is_category_24_product was defined earlier in the loop for this $line
								if ($is_category_24_product) {
									$batch_input_value = dol_escape_htmltag($object->ref); // Use MO reference
									$batch_input_readonly = 'readonly="readonly"';
								}
								
								print '<input type="text" class="width75" name="batchtoproduce-'.$line->id.'-'.$i.'" value="'.$batch_input_value.'" '.$batch_input_readonly.'>';
							}
							print '</td>';
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    // These rows now have data-product-id and data-mo-ref attributes from the previous change.
    $('tr[data-mo-ref]').each(function() {
        var $row = $(this);
        var productId = $row.data('product-id');
        var moRefFromDataAttr = $row.data('mo-ref'); // mo_ref from tr data attribute

        var $inputField = $row.find('input[name^="batchtoproduce-"]');

        if ($inputField.length) {
            if (productId == 31) {
                var lineIdInputName = $inputField.attr('name'); 
                var lineIdMatch = lineIdInputName.match(/batchtoproduce-(\d+)-/);
                
                if (lineIdMatch && lineIdMatch[1]) {
                    var lineId = lineIdMatch[1];
                    // Try to get the total quantity for this specific "toproduce" line using the hidden input qty_ordered_LINEID
                    var $qtyOrderedElement = $('#qty_ordered_' + lineId);
                    
                    if ($qtyOrderedElement.length > 0 && $qtyOrderedElement.val() !== "") {
                        var totalQtyForLine = parseFloat($qtyOrderedElement.val());

                        if (totalQtyForLine > 1) {
                            $inputField.val(''); // Clear value for auto-generation
                            $inputField.prop('readonly', true);
                            // Add info text if not already present
                        ...
                        } else { // totalQtyForLine is 1 or less (or not a number)
                            // Use MO ref as serial for qty = 1
                            if (typeof moRefFromDataAttr !== 'undefined' && moRefFromDataAttr) {
                                $inputField.val(moRefFromDataAttr);
                            } else if (typeof currentMoRef !== 'undefined') { // Fallback to global currentMoRef if data-mo-ref is missing
                                 $inputField.val(currentMoRef);
                            } else {
                                console.error("JavaScript: MO ref for product 31 (qty=1) is not defined.");
                            }
                            $inputField.prop('readonly', true);
                            $inputField.next('.auto-serial-info').remove(); // Remove info text if present
                        }
                    } else { // Fallback if qty_ordered_LINEID is not found or empty - treat as qty 1
                        if (typeof moRefFromDataAttr !== 'undefined' && moRefFromDataAttr) {
                            $inputField.val(moRefFromDataAttr);
                        ...
                        $inputField.prop('readonly', true);
                        console.warn("JavaScript: Could not find #qty_ordered_" + lineId + " or it was empty for product 31. Defaulting to MO ref for batch.");
                        $inputField.next('.auto-serial-info').remove();
                    }
                } else { // Fallback if lineId cannot be parsed - should ideally not happen
                     if (typeof moRefFromDataAttr !== 'undefined' && moRefFromDataAttr) {
                            $inputField.val(moRefFromDataAttr);
                     } else if (typeof currentMoRef !== 'undefined') {
                         $inputField.val(currentMoRef);
                     ...
                     $inputField.prop('readonly', true);
                     console.warn("JavaScript: Could not parse lineId from input name for product 31. Defaulting to MO ref for batch.");
                     $inputField.next('.auto-serial-info').remove();
                }
            } else { // Not product 31
                $inputField.prop('readonly', false); // Ensure it's not readonly
                $inputField.next('.auto-serial-info').remove(); // Remove info text if present
            }
        }
    });

...
// Function to get custom warehouse options
function getCustomWarehouseOptionsHtml($db, $selected_id) {
	global $conf, $langs;
	$langs->load("stocks");

	$out = '';
	$sql = "SELECT e.rowid, e.ref as label, e.description";
	$sql .= " FROM ".MAIN_DB_PREFIX."entrepot as e";
	$sql .= " WHERE e.entity IN (".getEntity('stock').") AND e.statut = 1 AND e.rowid = 4";
	$sql .= " ORDER BY e.ref";
	...
	if ($resql) {
		...
		if ($num > 0) {
			while ($obj = $db->fetch_object($resql)) {
				$label = $langs->trans("Warehouse").' '.$obj->label;
				if ($obj->description) {
					$label .= ' ('.$obj->description.')';
				}
				$out .= '<option value="'.$obj->rowid.'"'.($selected_id == $obj->rowid ? ' selected="selected"' : '').'>';
				$out .= dol_escape_htmltag($label);
				$out .= '</option>';
			}
		} else {
			// If warehouse 4 is not found or inactive, provide a non-selectable option or a message.
			// For now, it will result in an empty select if not found, or a select with only warehouse 4.
			// Consider adding a "No specific warehouse" or error message if warehouse 4 isn't available.
		}
		$db->free($resql);
	} else {
		dol_print_error($db);
...
if ($action == 'create') {
	...
	// Common attributes
	// Make fk_soc mandatory
	if (isset($object->fields['fk_soc'])) {
		$object->fields['fk_soc']['notnull'] = 1;
	} else {
		// If fk_soc is not in the fields array, add a basic definition
		// This is less likely for a common field but ensures robustness
		$object->fields['fk_soc'] = array(
			'type' => 'integer:Societe:societe/class/societe.class.php',
			'label' => 'ThirdParty',
			'notnull' => 1,
			'enabled' => 1, // Ensure it's enabled
			'visible' => 1  // Ensure it's visible
		);
	}

	// Custom handling for fk_warehouse
	$original_fk_warehouse_field_create = null;
	if (isset($object->fields['fk_warehouse'])) {
		$original_fk_warehouse_field_create = $object->fields['fk_warehouse']; // Save original
		unset($object->fields['fk_warehouse']); // Remove from automatic processing by commonfields_add
	}

	include DOL_DOCUMENT_ROOT.'/core/tpl/commonfields_add.tpl.php';

	// Manually render fk_warehouse if it was originally there
	if ($original_fk_warehouse_field_create) {
		print '<tr class="field_fk_warehouse oddeven">'; // Adding oddeven class for styling consistency
		print '<td class="titlefieldcreate'.(isset($original_fk_warehouse_field_create['notnull']) && $original_fk_warehouse_field_create['notnull'] > 0 ? ' fieldrequired' : '').'">';
		print $form->textwithpicto($langs->trans($original_fk_warehouse_field_create['label']), !empty($original_fk_warehouse_field_create['help']) ? $langs->trans($original_fk_warehouse_field_create['help']) : '');
		print '</td><td class="valuefieldcreate">';
		$options_html = getCustomWarehouseOptionsHtml($db, GETPOST('fk_warehouse'));
		print '<select name="fk_warehouse" id="fk_warehouse" class="flat minwidth200'.(isset($original_fk_warehouse_field_create['notnull']) && $original_fk_warehouse_field_create['notnull'] > 0 ? ' required' : '').'">' . $options_html . '</select>';
		print '</td></tr>';
	}
...
if (($id || $ref) && $action == 'edit') {
	...
	// Common attributes
	// Custom handling for fk_warehouse in edit mode
	$original_fk_warehouse_field_edit = null;
	if (isset($object->fields['fk_warehouse'])) {
		$original_fk_warehouse_field_edit = $object->fields['fk_warehouse'];
		$object->fields['fk_warehouse']['visible'] = 0; // Hide from commonfields_edit.tpl.php
	}
	...
	// Manual render for fk_warehouse in EDIT mode - placed just before closing the table for common fields
	if ($original_fk_warehouse_field_edit) {
		print '<tr class="field_fk_warehouse oddeven">'; // Adding oddeven class for styling consistency
		print '<td class="titlefield'.(isset($original_fk_warehouse_field_edit['notnull']) && $original_fk_warehouse_field_edit['notnull'] > 0 ? ' fieldrequired' : '').'">';
		print $form->textwithpicto($langs->trans($original_fk_warehouse_field_edit['label']), !empty($original_fk_warehouse_field_edit['help']) ? $langs->trans($original_fk_warehouse_field_edit['help']) : '');
		print '</td><td class="valuefield">';
		$options_html = getCustomWarehouseOptionsHtml($db, $object->fk_warehouse);
		// Ensure $object->form is available or use a new Form instance if needed.
		// It's typically available if commonfields_edit.tpl.php or Form class was used.
		// For safety, can check: if (empty($object->form)) $object->form = new Form($db);
		// However, $form is already instantiated globally in this script. So $form can be used.
		print '<select name="fk_warehouse" id="fk_warehouse" class="flat minwidth200'.(isset($original_fk_warehouse_field_edit['notnull']) && $original_fk_warehouse_field_edit['notnull'] > 0 ? ' required' : '').'">' . $options_html . '</select>';
		// The call to $object->form->showInputFieldEnd(); might not be necessary if we are just printing a select.
		// It's usually for complex fields. Removing it for a simple select.
		print '</td></tr>';
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			// Back to draft
		if ($object->status == $object::STATUS_VALIDATED && $user->admin) {
    print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=confirm_setdraft&confirm=yes&token='.newToken().'">'.$langs->trans("SetToDraft").'</a>';
			...
			// Delete
			if ($object->status == Mo::STATUS_PRODUCED && !$user->admin) {
				print dolGetButtonAction($langs->trans("Delete"), '', 'delete', $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=delete&token='.newToken(), 'delete', 0);
			} else {

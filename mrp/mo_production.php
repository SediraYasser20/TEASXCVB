	...
	*/
    ...
    if ($user->admin) {
        ...
        print '<a class="butActionRefused classfortooltip" href="#" title="'.$langs->trans("OnlyAdminCanReopen").'">'.$langs->trans('ReOpen').'</a>';
			...
			// Add "Start" button if status is Mo::STATUS_INPROGRESS (displays as "Shipped")
			if ($object->status == Mo::STATUS_INPROGRESS && $permissiontoproduce) { // Assuming same permission as consume/produce
				print '<a class="butAction" href="#" id="startButton">'.$langs->trans('Start').'</a>';
			}
		}
...
jQuery(document).ready(function() {
    jQuery('#startButton').on('click', function(e) {
        ...
        jQuery(this).hide(); // Hide the Start button itself
    });
});
</script>
<?php

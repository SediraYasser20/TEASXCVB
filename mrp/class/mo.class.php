 ...
 * Copyright (C) 2023-2024  Fr̩d̩ric France     <frederic.france@free.fr>
...
public function create(User $user, $notrigger = 0)
{
    ...
    if ($this->fk_bom > 0) {
        // If there is a known BOM, we force the type of MO to the type of BOM and calculate manufacturing cost
        include_once DOL_DOCUMENT_ROOT.'/bom/class/bom.class.php';
        $tmpbom = new BOM($this->db);
        $res = $tmpbom->fetch($this->fk_bom);
        if ($res > 0) {
            $this->mrptype = $tmpbom->bomtype;
            $tmpbom->calculateCosts();
            $this->manufacturing_cost = (float) price2num($tmpbom->total_cost * $this->qty, 'MT');
        } else {
            $this->error = $tmpbom->error ?: 'Failed to fetch BOM with ID ' . $this->fk_bom;
            $this->errors[] = $this->error;
            $this->db->rollback();
            return -1;
        }
    } else {
        $this->manufacturing_cost = 0;
    }
    ...
    if (!$error && $idcreated > 0) {
        // Update manufacturing_cost in the database
        $sql = "UPDATE ".MAIN_DB_PREFIX."mrp_mo SET manufacturing_cost = ".$this->db->escape($this->manufacturing_cost)." WHERE rowid = ".((int) $idcreated);
...
public function createProduction(User $user, $notrigger = 0)
{
    ...
    // Delete existing production lines for this MO
    if (!$error) {
        ...
        // Set manufacturing price from PMP if price not set
        if (empty($moline->price) || $moline->price <= 0) {
            $moline->price = $tmpproduct->pmp > 0 ? $tmpproduct->pmp : 0;
        }
        ...
        // If BOM is defined, create consumption lines
        if ($this->fk_bom > 0 && is_object($bom)) {
            if ($bom->id > 0) {
                if (!$error) {
                    foreach ($bom->lines as $line) {
                        ...
                        if ($line->qty_frozen) {
                            $moline->qty = $line->qty; // Qty to consume does not depend on qty to produce
                        } else {
                            $moline->qty = (float) price2num(
                                ($line->qty / (!empty($bom->qty) ? $bom->qty : 1)) * $this->qty / (!empty($line->efficiency) ? $line->efficiency : 1),
                                'MS'
                            ); // Calculate with Qty to produce and more precision
                        }
                        if ($moline->qty == 0) {
                            $error++;
                            $this->error = "Quantity to consume for a material cannot be 0";
                            $this->errors[] = $this->error;
                            break;
                        }
                        ...
                        // If quantity is negative, create a corresponding 'toproduce' line with positive quantity
                        if ($moline->qty < 0) {
                            $produce_moline = new MoLine($this->db);
                            $produce_moline->fk_mo = $this->id;
                            $produce_moline->origin_id = $line->id;
                            $produce_moline->origin_type = 'bomline';
                            $produce_moline->fk_unit = $line->fk_unit;
                            $produce_moline->qty = abs($moline->qty); // Use absolute value
                            $produce_moline->fk_product = $line->fk_product;
                            $produce_moline->role = 'toproduce';
                            $produce_moline->position = $line->position + 1; // Offset position to avoid conflicts
                            $produce_moline->qty_frozen = $line->qty_frozen;
                            $produce_moline->disable_stock_change = $line->disable_stock_change;
                            if (!empty($line->fk_default_workstation)) {
                                $produce_moline->fk_default_workstation = $line->fk_default_workstation;
                            }
                            // Add a comment to indicate this is a by-product
                            $produce_moline->description = 'By-product (negative quantity from BOM)';

                            // Fetch product PMP and set price if zero
                            $tmpproduct = new Product($this->db);
                            $tmpproduct->fetch($produce_moline->fk_product);

                            if (empty($produce_moline->price) || $produce_moline->price <= 0) {
                                $produce_moline->price = $tmpproduct->pmp > 0 ? $tmpproduct->pmp : 0;
                            }

                            $produce_moline->array_options['options_byproduct_price'] = !empty($tmpproduct->pmp) ? price2num($tmpproduct->pmp, 'MU') : price2num($tmpproduct->cost_price, 'MU');

                            $resultline = $produce_moline->create($user, false); // Never use triggers here
                            if ($resultline <= 0) {
                                $error++;
                                $this->error = $produce_moline->error;
                                $this->errors = array_merge($this->errors, $produce_moline->errors);
                                dol_print_error($this->db, $produce_moline->error, $produce_moline->errors);
                                break;
                            }
                        }
                    }
	...
	public function printOriginLine($line, $var, $restrictlist = '', $defaulttpldir = '/core/tpl', $selectedLines = array())
	{
		...
		if (!empty($line->fk_product) && $line->fk_product > 0) {
			...
			$productstatic->load_stock(''); // Explicitly call load_stock to ensure stock_reel is filtered
			$productstatic->load_virtual_stock(); // This will now use the filtered stock_reel
...
public function getCalculatedUnitCost()
{
    global $conf, $langs; // Add if needed for translations or specific conf values

    // Ensure qty is valid for division, default to 1 if 0 to avoid division by zero for unit cost.
    $effective_qty = ($this->qty > 0 ? $this->qty : 1);

    // 1. Prioritize manufacturing_cost if set and greater than 0
    if (isset($this->manufacturing_cost) && $this->manufacturing_cost > 0) {
        return price2num($this->manufacturing_cost / $effective_qty, 'MU');
    }

    // 2. Fallback to BOM calculation if manufacturing_cost is not set, or is explicitly 0
    // (allowing manufacturing_cost=0 to trigger BOM calculation)
    if (empty($this->lines) && method_exists($this, 'fetchLines')) {
        $this->fetchLines(); // Make sure lines are loaded
    }
    ...
    if (is_array($this->lines)) {
        foreach ($this->lines as $line) {
            if ($line->role == 'toconsume' && !empty($line->fk_product)) {
                // DOL_DOCUMENT_ROOT.'/product/class/product.class.php' is already included via moline.class.php
                $component_product = new Product($this->db);
                $component_product->fetch($line->fk_product);

                $component_unit_cost = 0;
                if (isset($component_product->cost_price) && $component_product->cost_price > 0) {
                    $component_unit_cost = $component_product->cost_price;
                } elseif (isset($component_product->pmp) && $component_product->pmp > 0) {
                    $component_unit_cost = $component_product->pmp;
                } else {
                    // Fallback to supplier price if cost_price and PMP are not available
                    if (isModEnabled('supplier') || isModEnabled('fournisseur')) { // Check if supplier module is enabled
                        require_once DOL_DOCUMENT_ROOT . '/fourn/class/fournisseur.product.class.php';
                        $productFournisseur = new ProductFournisseur($this->db);
                        // find_min_price_product_fournisseur returns > 0 on success
                        if ($productFournisseur->find_min_price_product_fournisseur($line->fk_product, $line->qty) > 0) {
                            $component_unit_cost = $productFournisseur->fourn_unitprice;
                        }
                    }
                }
                $calculated_bom_cost += price2num($line->qty * $component_unit_cost, 'MU');
            }
        }
    }

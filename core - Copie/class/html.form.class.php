...
public function select_type_of_lines($selected = '', $htmlname = 'type', $showempty = 0, $hidetext = 0, $forceall = 0, $morecss = "", $useajaxcombo = 1)
{
    ...
    if ($forceall == 1 || (empty($forceall) && isModEnabled("product") && isModEnabled("service"))
        ...
        print '>' . $langs->trans("Product") . '</option>';  // <-- missing closing option tag in your snippet, added here
        ...
        // Fabrication order option (NEW)
        print '<option value="2"';
        if (2 == $selected) {
            print ' selected';
        }
        print '>' . $langs->trans("FabricationOrder") . '</option>';
...
$sqlMo .= " WHERE status = " . ((int) Mo::STATUS_VALIDATED); // Status = 1 for validated MOs
$sqlMo .= " AND entity IN (" . getEntity('mrp_mo') . ")"; // Entity filter for multi-company
dol_syslog(get_class($this) . "::select_produits_list search manufacturing orders", LOG_DEBUG);

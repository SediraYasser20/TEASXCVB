...
if ($action == 'BILL_VALIDATE') {
    ...
    if (isModEnabled('order') && !empty($conf->workflow->enabled) && getDolGlobalString('WORKFLOW_INVOICE_AMOUNT_CLASSIFY_BILLED_ORDER')) {
        ...
        if (!empty($object->linkedObjects['commande'])) {
            ...
            // Modified condition: Allow classification if invoice amount >= order amount
            $shouldClassifyOrder = false;
            if (getDolGlobalString('WORKFLOW_CLASSIFY_IF_AMOUNTS_ARE_DIFFERENTS')) {
                $shouldClassifyOrder = true;
            } else {
                // Allow classification if invoice amount is greater than or equal to order amount
                $shouldClassifyOrder = (price2num($object->total_ht, 'MT') >= price2num($totalonlinkedelements, 'MT'));
            }
            
            if ($shouldClassifyOrder) {
                foreach ($object->linkedObjects['commande'] as $element) {
    ...
    // Rest of the BILL_VALIDATE logic remains the same...
    // [Include all the other sections like shipping, etc.]
...
private function shouldClassify($conf, $totalonlinkedelements, $object_total_ht)
{
    ...
    // Allow classification if invoice amount is greater than or equal to order amount
    // This handles cases where additional items/charges are added to the invoice

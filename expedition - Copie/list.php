...
$arrayfields = array(
	...
	'e.billed' => array('label' => $langs->trans("BilledOrder"), 'checked' => 1, 'position' => 1100, 'enabled' => 1, 'help' => $langs->trans("BilledOrderHelp")),
...
$sql .= " e.rowid, e.ref, e.ref_customer, e.date_expedition as date_expedition, e.weight, e.weight_units, e.date_expedition, e.date_delivery as delivery_date, e.fk_statut, e.signed_status, (
    EXISTS (
        SELECT 1
        FROM ".MAIN_DB_PREFIX."element_element AS ee_cmd
        INNER JOIN ".MAIN_DB_PREFIX."element_element AS ee_fact
        ON ee_cmd.fk_source = ee_fact.fk_source
        AND ee_cmd.sourcetype = 'commande'
        AND ee_fact.sourcetype = 'commande'
        AND ee_fact.targettype = 'facture'
        WHERE ee_cmd.fk_target = e.rowid
        AND ee_cmd.targettype = 'shipping'
    )
) AS billed, e.tracking_number, e.fk_shipping_method,";
...
if (!empty($arrayfields['e.billed']['checked'])) {
    print '<td class="liste_titre maxwidthonsmartphone center">';
    print $form->selectyesno('search_billed', $search_billed, 1, 0, 1, 0, '', '', '', 'title="'.$langs->trans("BilledOrderHelp").'"');
    print '</td>';
}
// Action column
if (!getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {

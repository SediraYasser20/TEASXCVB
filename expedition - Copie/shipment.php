...
if (isModEnabled('stock') && ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED)) {
    if ($user->hasRight('expedition', 'creer')) {
        ...
        if (isModEnabled('stock')) {
            ...
            $user_warehouse_id = $user->fk_warehouse;
if (!empty($user_warehouse_id) && $user_warehouse_id > 0) {

                // Query the user's default warehouse
                $sql = "SELECT rowid, ref, lieu, statut, entity";
                $sql .= " FROM ".MAIN_DB_PREFIX."entrepot";
                $sql .= " WHERE rowid = ".((int) $user_warehouse_id);
                ...
                if ($resql && $db->num_rows($resql) > 0) {
                    ...
                    $warehouse_options[$obj->rowid] = $obj->ref.' ('.$obj->lieu.')';
                    // Update cache_warehouses to avoid "No warehouse defined" message
                    $formproduct->cache_warehouses[$obj->rowid] = array(
                        'rowid' => $obj->rowid,
                        'ref' => $obj->ref,
                        'lieu' => $obj->lieu,
                        'statut' => $obj->statut,
                ...
                if (!empty($warehouse_options)) {
                    // Generate dropdown with the user's warehouse
                    print $form->selectarray('entrepot_id', $warehouse_options, $user_warehouse_id, 1, 0, 0, '', 0, 0, 0, 'minwidth200');
                } else {
                    print '<span class="error">'.$langs->trans("ErrorNoValidWarehouseAssigned").' (ID: '.$user_warehouse_id.')</span>';
                }
            } else {
                // No default warehouse set for user
                print '<span class="warning">'.$langs->trans("NoDefaultWarehouseAssigned").'</span>';
                // Fallback to all warehouses (optional, remove if you want to block shipment creation)
            ...
            // Check if any warehouses are available to avoid "No warehouse defined" message
            if (empty($formproduct->cache_warehouses)) {
                print ' <span class="error">'.$langs->trans("WarehouseSourceNotDefined").'</span>';
                print ' <a href="'.DOL_URL_ROOT.'/product/stock/card.php?action=create">'.$langs->trans("AddOne").'</a>';
            }
        }
        print ' <input type="submit" class="butAction" name="save" value="'.$langs->trans("CreateShipment").'">';
        if ($toBeShippedTotal <= 0) {
            print ' '.img_warning($langs->trans("WarningNoQtyLeftToSend"));
        }
        print "</form>\n";

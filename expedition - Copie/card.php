        ...
        if ($line->fk_product > 0) {
            ...
            if (!empty($user_warehouse_id)) {
                ...
                if ($resql && $obj = $db->fetch_object($resql)) {
                    ...
                    $warehouse_options = array('' => $langs->trans("NoValidWarehouseAssigned"));
                }
                $db->free($resql);

                // Generate dropdown with only the user's warehouse
                print $form->selectarray('entl'.$indiceAsked, $warehouse_options, $tmpentrepot_id, 1, 0, 0, '', 0, 0, 0, 'minwidth200');
            } else {
                // Fallback if no default warehouse is assigned
                $stockMin = false;
...
if (($user->socid == 0) && ($action != 'presend')) {
    ...
    if (empty($reshook)) {
        ...
        if ($object->status == Expedition::STATUS_VALIDATED && !getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT')) {
            // Back to draft only available for admin users
            if ($user->admin) {
        ...
        if ($object->status == Expedition::STATUS_CLOSED) {
            // Re-Open only available for admin users
            if ($user->admin) {
        ...
        // Create bill - only available for admin users
        ...
        if ($object->status == Expedition::STATUS_VALIDATED) {
            // ClassifyBilled button has been hidden as requested
            
            // Close button only for admin users or users with poste="5" (RESPONSBLE-D)
            $sql = "SELECT poste FROM " . MAIN_DB_PREFIX . "user_extrafields WHERE fk_object = " . $user->id;
            $resql = $db->query($sql);
            $canClose = false;
            
            if ($resql && $db->num_rows($resql) > 0) {
                $obj = $db->fetch_object($resql);
                if (!empty($obj->poste) && $obj->poste == "5") {
                    $canClose = true;
                }
            }
            
            if ($user->admin || $canClose) {
                print dolGetButtonAction('', $langs->trans("Close"), 'default', $_SERVER["PHP_SELF"].'?action=classifyclosed&token='.newToken().'&id='.$object->id, '');
            }
        }

        // Cancel - only available for admin users
        if ($object->status == Expedition::STATUS_VALIDATED) {
            if ($user->admin) {
        ...
        // Delete - only available for admin users
        if ($user->admin) {

...
if ($action == 'fetch' && !empty($id)) {
	...
	$ret = $object->fetch($id);
	$object->load_stock(''); // Ensures stock_reel is calculated excluding WH7 and stock_warehouse is populated.
	$object->load_virtual_stock(); // Ensures stock_theorique is calculated based on the adjusted stock_reel.
	if ($ret > 0) {
		$outref = $object->ref;

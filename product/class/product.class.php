...
class Product extends CommonObject
{
	...
	public function fetch($id = 0, $ref = '', $ref_ext = '', $barcode = '', $ignore_expression = 0, $ignore_price_load = 0, $ignore_lang_load = 0)
	{
		...
		if ($separatedStock) {
			$sql .= " LEFT JOIN " . $this->db->prefix() . "product_stock as sp ON (sp.fk_product = p.rowid AND sp.fk_entrepot <> 2 AND sp.fk_entrepot IN (SELECT rowid FROM ".$this->db->prefix()."entrepot WHERE entity IN (".$this->db->sanitize($visibleWarehousesEntities).")) )";
		}
		...
		if ($result) {
			...
			if ($num > 0) {
				while ($i < $num) {
					...
					if ($row->fk_entrepot != 2) { // Changed from 7 to 2

...
class Form
{
		...
		public function select_produits_list($selected = 0, $htmlname = 'productid', $filtertype = '', $limit = 20, $price_level = 0, $filterkey = '', $status = 1, $finished = 2, $outputmode = 0, $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = 'maxwidth500', $hidepriceinlabel = 0, $warehouseStatus = '', $status_purchase = -1, $warehouseId = 0)
	{
		...
		// Always calculate stock by summing from llx_product_stock, excluding warehouse 7,
		// and optionally filtering by warehouse status using a subquery.
		$subQueryStock = "(SELECT SUM(ps_sum.reel) FROM ".$this->db->prefix()."product_stock as ps_sum";
		if (count($warehouseStatusArray)) {
			// If filtering by warehouse status, join with entrepot inside the subquery
			// Note: Using array_map for escape if $this->db->escape handles arrays, otherwise loop. Assuming $this->db->escape is for single strings.
			$escapedStatuses = array();
			foreach ($warehouseStatusArray as $statusItem) {
				$escapedStatuses[] = $this->db->escape($statusItem);
			}
			$subQueryStock .= " INNER JOIN ".$this->db->prefix()."entrepot as w_sum ON ps_sum.fk_entrepot = w_sum.rowid AND w_sum.entity IN (".getEntity('stock').") AND w_sum.statut IN ('".implode("','", $escapedStatuses)."')";
		}
		$subQueryStock .= " WHERE ps_sum.fk_product = p.rowid AND ps_sum.fk_entrepot <> 2"; // Changed 7 to 2
		$subQueryStock .= ")";
		$selectFieldsGrouped = ", COALESCE(".$subQueryStock.", 0) as stock";
...
// Append validated Manufacturing Orders
// Append validated Manufacturing Orders
// Append validated Manufacturing Orders
require_once DOL_DOCUMENT_ROOT . '/mrp/class/mo.class.php'; // Ensure Mo class is available for constants
require_once DOL_DOCUMENT_ROOT . '/product/class/product.class.php'; // For fetching product costs in fallback
...
if ($resMo) {
    global $langs, $mysoc, $object; // Ensure $langs, $mysoc, and $object are available
    while ($mo = $this->db->fetch_object($resMo)) {
        $moKey = 'MO' . $mo->rowid; // Unique key to distinguish from product IDs
        
        // Calculate unit cost (price_ht) using manufacturing_cost or fallback
        $qty = $mo->qty > 0 ? $mo->qty : 1; // Avoid division by zero
        $price_ht = 0;
        if (!empty($mo->manufacturing_cost)) {
            $price_ht = price2num($mo->manufacturing_cost / $qty, 'MU'); // Unit cost
        } else {
            // Fallback: Recalculate cost based on consumed lines
            $moObject = new Mo($this->db);
            if ($moObject->fetch($mo->rowid) > 0) {
                $moObject->fetchLines();
                $bomcostupdated = 0;
                foreach ($moObject->lines as $line) {
                    if ($line->role == 'toconsume') {
                        ...
                        $bomcostupdated += price2num($line->qty * $costprice, 'MU');
                    }
                }
                $price_ht = price2num($bomcostupdated / $qty, 'MU'); // Unit cost
            }
        }

        // Calculate selling price (example: 20% margin)
        $selling_price = price2num($price_ht * 1.2, 'MU');
        
        // Calculate VAT rate and price TTC
if (is_object($object->thirdparty) && $object->thirdparty instanceof Societe) {
    $vat_rate = get_default_tva($mysoc, $object->thirdparty, $mo->fk_product);
    // Fetch default_vat_code from linked product
    $sql = "SELECT default_vat_code FROM " . $this->db->prefix() . "product WHERE rowid = " . ((int) $mo->fk_product);
    $resql = $this->db->query($sql);
    $default_vat_code = '';
    if ($resql && ($prod = $this->db->fetch_object($resql))) {
        $default_vat_code = $prod->default_vat_code ? $prod->default_vat_code : '';
    }
} else {
    // Fallback: Fetch VAT rate and default_vat_code from linked product
    $sql = "SELECT tva_tx, default_vat_code FROM " . $this->db->prefix() . "product WHERE rowid = " . ((int) $mo->fk_product);
    $resql = $this->db->query($sql);
    if ($resql && ($prod = $this->db->fetch_object($resql))) {
        $vat_rate = $prod->tva_tx ? $prod->tva_tx : getDolGlobalString('MAIN_DEFAULT_VAT_RATE', 20);
        $default_vat_code = $prod->default_vat_code ? $prod->default_vat_code : '';
    } else {
        $vat_rate = getDolGlobalString('MAIN_DEFAULT_VAT_RATE', 20);
        $default_vat_code = '';
    }
}
        ...
        $price_ttc = price2num($selling_price * (1 + ($vat_rate / 100)), 'MU');

        $label = $mo->ref . ' (Fabrication, Qty: ' . $mo->qty . ', Unit Price: ' . price($selling_price) . ' ' . $langs->trans('HT') . ')';

        // Build HTML <option> with data attributes
        $out .= '<option value="' . dol_escape_htmltag($moKey) . '"';
        $out .= ' data-type="mo"'; // Add data-type to identify MOs
        $out .= ' data-qty="' . dol_escape_htmltag($qty) . '"';
        $out .= ' data-price-ht="' . dol_escape_htmltag($selling_price) . '"'; // Selling price (excl. tax)
        $out .= ' data-price-ttc="' . dol_escape_htmltag($price_ttc) . '"'; // Selling price (incl. tax)
        $out .= ' data-tva_tx="' . dol_escape_htmltag($vat_rate) . '"'; // VAT rate
        $out .= ' data-cost_price="' . dol_escape_htmltag($price_ht) . '"'; // Unit cost price
		$out .= ' data-default-vat-code="' . dol_escape_htmltag($default_vat_code) . '"'; // Default VAT code
        $out .= '>' . dol_escape_htmltag($label) . '</option>' . "\n";
		

        // Build JSON entry for Ajax form handler
		
        $optJson = array(
            'key'        => $moKey,
            'value'      => $mo->ref,
            'label'      => $label,
            'desc'       => $mo->ref . ' (Fabrication, Qty: ' . $mo->qty . ')',
            'type'       => 'mo', // Custom type to indicate manufacturing order
            'qty'        => $qty, // Quantity from MO
            'price_ht'   => $selling_price, // Unit selling price (excl. tax)
            'price_ttc'  => $price_ttc, // Unit selling price (incl. tax)
            'tva_tx'     => $vat_rate, // VAT rate
            'cost_price' => $price_ht, // Unit cost price
            'subprice'   => $selling_price, // Legacy field for compatibility
			'default_vat_code' => $default_vat_code // Default VAT code
        );
        array_push($outarray, $optJson);
    }
    $this->db->free($resMo);

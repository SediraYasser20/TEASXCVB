...
/**
 * \file        class/receptionshowroom1.class.php
 * \ingroup     recshow
 * \brief       This file is a CRUD class file for Receptionshowroom1 (Create/Read/Update/Delete)
 */
...
/**
 * Class for Receptionshowroom1
 */
class Receptionshowroom1 extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'recshow';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'receptionshowroom1';
	 ...
	 */
	//public $element_for_permission = 'recshow';

	/**
	 * @var string 	String with name of icon for receptionshowroom1. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'receptionshowroom1@recshow' if picto is file 'img/object_receptionshowroom1.png'.
	 */
	...
	public $fields = array(
		...
		"ref" => array("type" => "varchar(128)", "label" => "Ref", "enabled" => "1", 'position' => 20, 'notnull' => 0, "visible" => "0", "noteditable" => "1", "index" => "1", "searchall" => "1", "validate" => "1", "comment" => "Reference of object"),
		...
		"note_private" => array("type" => "html", "label" => "NotePrivate", "enabled" => "1", 'position' => 62, 'notnull' => 0, "visible" => "1", "cssview" => "wordbreak", "validate" => "1",),
		...
		"gpu" => array("type" => "varchar(128)", "label" => "GPU", "enabled" => "1", 'position' => 8, 'notnull' => 0, "visible" => "1",),
		"motherboard" => array("type" => "varchar(128)", "label" => "Motherboard", "enabled" => "1", 'position' => 9, 'notnull' => 0, "visible" => "1",),
		"chargeur" => array("type" => "varchar(128)", "label" => "Chargeur", "enabled" => "1", 'position' => 10, 'notnull' => 0, "visible" => "1",),
	...
	//  */
	// public $table_element_line = 'recshow_receptionshowroom1line';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_receptionshowroom1';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Receptionshowroom1line';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Receptionshowroom1', 'fk_element'=>'fk_receptionshowroom1'));
	...
	//  */
	// protected $childtablesoncascade = array('recshow_receptionshowroom1det');

	// /**
	//  * @var Receptionshowroom1Line[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('recshow', 'receptionshowroom1', 'read')) {
...
public function getNextRef()
{
    ...
    $sql = "SELECT ref FROM " . MAIN_DB_PREFIX . $this->table_element . " 
            WHERE ref LIKE '" . $this->db->escape($prefix . $datePart . "-%") . "'
    ...
    $nextSeq = $lastSeq + 1;
    $nextSeqPadded = str_pad($nextSeq, 5, '0', STR_PAD_LEFT);
    $newRef = $prefix . $datePart . '-' . $nextSeqPadded;
...
public function create(User $user, $notrigger = 0)
{
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('recshow', 'receptionshowroom1', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('recshow', 'receptionshowroom1_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'receptionshowroom1/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'receptionshowroom1/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'receptionshowroom1/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'receptionshowroom1/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->recshow->dir_output.'/receptionshowroom1/'.$oldref;
				$dirdest = $conf->recshow->dir_output.'/receptionshowroom1/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->recshow->dir_output.'/receptionshowroom1/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('recshow','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('recshow','recshow_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'RECSHOW_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'RECSHOW_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'RECSHOW_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowReceptionshowroom1")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Receptionshowroom1").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/recshow/receptionshowroom1_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowReceptionshowroom1");
	...
	public function getLinesArray()
	{
		...
		$objectline = new Receptionshowroom1Line($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_receptionshowroom1:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('RECSHOW_MYOBJECT_ADDON')) {
			$conf->global->RECSHOW_MYOBJECT_ADDON = 'mod_receptionshowroom1_standard';
		}

		if (getDolGlobalString('RECSHOW_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('RECSHOW_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('RECSHOW_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/recshow/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefReceptionshowroom1 $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_receptionshowroom1';
		...
		$modelpath = "core/modules/recshow/doc/";
...
/**
 * Class Receptionshowroom1Line. You can also remove this and generate a CRUD class for lines objects.
 */
class Receptionshowroom1Line extends CommonObjectLine
{
	// To complete with content of an object Receptionshowroom1Line
	// We should have a field rowid, fk_receptionshowroom1 and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'receptionshowroom1'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_receptionshowroom1'

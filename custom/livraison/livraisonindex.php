...
$payeeOptions = fetchOptions($extraF, 'commande', 'payee'); // Fetch payee options
$payeeLabels = [
    '1' => $langs->trans('No'),
    '2' => $langs->trans('Yes'),
];
...
$shippingMethods = [];
$res = $db->query(
    "SELECT rowid, libelle FROM " . MAIN_DB_PREFIX . "c_shipment_mode WHERE active=1 ORDER BY libelle"
);
...
$users = [];
$res = $db->query(
    "SELECT rowid, firstname, lastname FROM " . MAIN_DB_PREFIX . "user WHERE statut=1 ORDER BY lastname, firstname"
);
...
$filters = [
    ...
    'payee' => GETPOST('payee_filter', 'int'), // Added payee filter
];

// Pagination
$limit = 15; // Entries per page
$page_orders = max(0, GETPOST('page_orders', 'int')); // Current page for orders (0-based)
$page_events = max(0, GETPOST('page_events', 'int')); // Current page for events (0-based)
...
// Build Orders WHERE
// ------------------------------------------------------------------------
// Fetch rowids for the specific shipping methods
...
if (!empty($shippingMethodIds)) {
    ...
    $where[] = "0=1"; // No matching shipping methods, return empty result
}
...
if ($f = $filters['payee']) $where[] = "e.payee=" . (int)$f; // Added payee filter
...
if (GETPOST('action') == 'csv') {
    ...
    $headers = [
        ...
        $langs->trans('Status'),
        $langs->trans('Payee'), // Added Payee column
        $langs->trans('ProcessingTime'),
        $langs->trans('AssignTo')
    ];
    fputcsv($output, $headers, ';');
    
    // Fetch all orders (no limit for export)
    $sqlExport = 
    ...
    while ($o = $db->fetch_object($resExport)) {
        ...
        $payee_label = isset($payeeLabels[$o->payee]) ? $payeeLabels[$o->payee] : $o->payee;
        $row = [
            $o->ref,
            $o->company,
            ...
            $payee_label, // Added Payee column
            $proc,
            $assigned_user
        ];
        fputcsv($output, $row, ';');
...
$total_orders = $db->fetch_object($resCount)->total;
$db->free($resCount);
...
while ($o = $db->fetch_object($resO)) {
    ...
    $orders[] = [
        ...
        'total_ht' => $o->total_ht, // Added for accurate summation
        'tracking' => $o->tracking,
...
$countSqlE = "SELECT COUNT(*) as total FROM " . MAIN_DB_PREFIX . "actioncomm AS a " .
    ...
    "WHERE a.entity = " . $conf->entity . " AND ef.towhom = '" . $db->escape('service-livraison') . "'";
...
$total_events = $db->fetch_object($resCountE)->total;
$db->free($resCountE);
...
$sqlE =
    ...
    "LEFT JOIN " . MAIN_DB_PREFIX . "actioncomm_extrafields AS ef ON ef.fk_object = a.id " .
    "LEFT JOIN " . MAIN_DB_PREFIX . "societe AS sc ON sc.rowid = a.fk_soc " .
    "WHERE a.entity = " . $conf->entity . " AND ef.towhom = '" . $db->escape($keyForDelivery) . "'";
...
foreach ($shippingMethods as $id => $lbl) {
    print '<option value="' . $id . '"' . ($filters['shipping_method'] == $id ? ' selected' : '') . '>' . dol_escape_htmltag($lbl) . '</option>';
}
print '</select></td>';
...
foreach ($destinationOptions as $k => $l) {
    print '<option value="' . $k . '"' . ($filters['destination'] == $k ? ' selected' : '') . '>' . dol_escape_htmltag($l) . '</option>';
}
print '</select></td>';
print '</tr><tr>';
print '<td>' . $langs->trans('Status') . ': <select name="status"><option value="">' . $langs->trans('All') . '</option>';
foreach ($statusOptions as $code => $lbl) {
    print '<option value="' . $code . '"' . ($filters['status'] == $code ? ' selected' : '') . '>' . dol_escape_htmltag($lbl) . '</option>';
}
print '</select></td>';
print '<td>' . $langs->trans('Payee') . ': <select name="payee_filter"><option value="">' . $langs->trans('All') . '</option>';
foreach ($payeeLabels as $code => $lbl) {
    print '<option value="' . $code . '"' . ($filters['payee'] == $code ? ' selected' : '') . '>' . dol_escape_htmltag($lbl) . '</option>';
}
print '</select></td>';
print '<td>' . $langs->trans('AssignTo') . ': <select name="assigned_user"><option value="">' . $langs->trans('All') . '</option>';
foreach ($users as $id => $name) {
    print '<option value="' . $id . '"' . ($filters['assigned_user'] == $id ? ' selected' : '') . '>' . dol_escape_htmltag($name) . '</option>';
}
print '</select></td>';
print '</tr><tr>';
print '<td colspan="3" style="text-align:right;">'
    . '<input type="submit" class="button" value="' . $langs->trans('Filter') . '"> '
    . '<a class="button" href="' . $_SERVER['PHP_SELF'] . '?' . http_build_query(array_merge($filters, ['action' => 'csv'])) . '">' . $langs->trans('ExportCSV') . '</a>'
    . '</td>';
...
if (count($orders)) {
    ...
    $sum = 0;
    foreach ($orders as $o) {
        $total_ht = is_numeric($o['total_ht']) ? (float)$o['total_ht'] : 0; // Use raw total_ht
        $sum += $total_ht;
        print '<tr>';
        print '<td><a href="' . $o['link'] . '">' . $o['ref'] . '</a></td>';
        print '<td>' . dol_escape_htmltag($o['company']) . '</td>';
        print '<td>' . dol_print_date($o['date'], 'dayhour') . '</td>';
        print '<td style="text-align:right;">' . $o['amount'] . '</td>';
        ...
        print '<td><span style="color:' . $o['color'] . ';font-weight:bold;">' . $o['status'] . '</span></td>';
        print '<td>' . dol_escape_htmltag($o['payee']) . '</td>'; // Added Payee column
        print '<td style="text-align:right;">' . $o['processing'] . '</td>';
    ...
    print '<td colspan="7"></td>'; // Adjusted colspan for new Payee column
    print '</tr>';
    print '</table>';
    ...
    if ($total_pages_orders > 1) {
        ...
        if ($page_orders > 0) {
            print '<a href="' . $base_url . '&page_orders=' . ($page_orders - 1) . '" class="button">' . $langs->trans('Previous') . '</a> ';
        }
        for ($i = 0; $i < $total_pages_orders; $i++) {
            if ($i == $page_orders) {
                print '<span style="font-weight:bold;">' . ($i + 1) . '</span> ';
            } else {
                print '<a href="' . $base_url . '&page_orders=' . $i . '">' . ($i + 1) . '</a> ';
            }
        }
        if ($page_orders < $total_pages_orders - 1) {
            print '<a href="' . $base_url . '&page_orders=' . ($page_orders + 1) . '" class="button">' . $langs->trans('Next') . '</a>';
        }
        print '</div>';
    }
    print '<br>';
...
if (count($events)) {
    ...
    foreach ($events as $e) {
        print '<tr>';
        print '<td><a href="' . $e['link'] . '">' . $e['ref'] . '</a></td>';
        ...
        $percent = is_numeric($e['percent']) ? (int)$e['percent'] : 0; // Fix for percent display
    ...
    if ($total_pages_events > 1) {
        ...
        if ($page_events > 0) {
            print '<a href="' . $base_url . '&page_events=' . ($page_events - 1) . '" class="button">' . $langs->trans('Previous') . '</a> ';
        }
        for ($i = 0; $i < $total_pages_events; $i++) {
            if ($i == $page_events) {
                print '<span style="font-weight:bold;">' . ($i + 1) . '</span> ';
            } else {
                print '<a href="' . $base_url . '&page_events=' . $i . '">' . ($i + 1) . '</a> ';
            }
        }
        if ($page_events < $total_pages_events - 1) {
            print '<a href="' . $base_url . '&page_events=' . ($page_events + 1) . '" class="button">' . $langs->trans('Next') . '</a>';
        }
        print '</div>';
    }
} else {

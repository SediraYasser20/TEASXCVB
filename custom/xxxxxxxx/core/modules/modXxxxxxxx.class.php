...
/**
 * 	\defgroup   xxxxxxxx     Module Xxxxxxxx
 *  \brief      Xxxxxxxx module descriptor.
 *
 *  \file       htdocs/xxxxxxxx/core/modules/modXxxxxxxx.class.php
 *  \ingroup    xxxxxxxx
 *  \brief      Description and activation file for module Xxxxxxxx
 */
include_once DOL_DOCUMENT_ROOT.'/core/modules/DolibarrModules.class.php';


/**
 *  Description and activation class for module Xxxxxxxx
 */
class modXxxxxxxx extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleXxxxxxxxName' not found (Xxxxxxxx is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));

		// DESCRIPTION_FLAG
		// Module description, used if translation string 'ModuleXxxxxxxxDesc' not found (Xxxxxxxx is name of module).
		$this->description = "XxxxxxxxDescription";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "XxxxxxxxDescription";

		// Author
		$this->editor_name = 'Informatics-dz';
		$this->editor_url = '';		// Must be an external online web site
		$this->editor_squarred_logo = '';					// Must be image filename into the module/img directory followed with @modulename. Example: 'myimage.png@xxxxxxxx'
		...
		// Key used in llx_const table to save module status enabled/disabled (where XXXXXXXX is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array(
				//    '/xxxxxxxx/css/xxxxxxxx.css.php',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				//   '/xxxxxxxx/js/xxxxxxxx.js.php',
			),
		...
		// Example: this->dirs = array("/xxxxxxxx/temp","/xxxxxxxx/subdir");
		$this->dirs = array("/xxxxxxxx/temp");

		// Config pages. Put here list of php page, stored into xxxxxxxx/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@xxxxxxxx");

		// Dependencies
		// A condition to hide module
		$this->hidden = getDolGlobalInt('MODULE_XXXXXXXX_DISABLED'); // A condition to disable module;
		...
		$this->langfiles = array("xxxxxxxx@xxxxxxxx");
		...
		//$this->automatic_activation = array('FR'=>'XxxxxxxxWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(1 => array('XXXXXXXX_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
		//                             2 => array('XXXXXXXX_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
		// );
		$this->const = array();
		...
		if (!isModEnabled("xxxxxxxx")) {
			$conf->xxxxxxxx = new stdClass();
			$conf->xxxxxxxx->enabled = 0;
		}
		...
		// $this->tabs[] = array('data' => 'objecttype:+tabname1:Title1:mylangfile@xxxxxxxx:$user->hasRight(\'xxxxxxxx\', \'read\'):/xxxxxxxx/mynewtab1.php?id=__ID__');
		// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		// $this->tabs[] = array('data' => 'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@xxxxxxxx:$user->hasRight(\'othermodule\', \'read\'):/xxxxxxxx/mynewtab2.php?id=__ID__',
		...
		/* Example:
		 ...
		 'tabcond' => array(isModEnabled('xxxxxxxx'), isModEnabled('xxxxxxxx'), isModEnabled('xxxxxxxx')),
		...
		// Boxes/Widgets
		// Add here list of php file(s) stored in xxxxxxxx/core/boxes that contains a class to show a widget.
		/* BEGIN MODULEBUILDER WIDGETS */
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'xxxxxxxxwidget1.php@xxxxxxxx',
			//      'note' => 'Widget provided by Xxxxxxxx',
		...
		$this->cronjobs = array(
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => 'isModEnabled("xxxxxxxx")',
			//      'priority' => 50,
			//  ),
		);
		/* END MODULEBUILDER CRON */
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'isModEnabled("xxxxxxxx")', 'priority'=>50),
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'isModEnabled("xxxxxxxx")', 'priority'=>50)
		// );
	...
	public function init($options = '')
	{
		...
		//$result = $this->_load_tables('/install/mysql/', 'xxxxxxxx');
		$result = $this->_load_tables('/xxxxxxxx/sql/');
		if ($result < 0) {
		...
		//$result0=$extrafields->addExtraField('xxxxxxxx_separator1', "Separator 1", 'separator', 1,  0, 'thirdparty',   0, 0, '', array('options'=>array(1=>1)), 1, '', 1, 0, '', '', 'xxxxxxxx@xxxxxxxx', 'isModEnabled("xxxxxxxx")');
		//$result1=$extrafields->addExtraField('xxxxxxxx_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', -1, 0, '', '', 'xxxxxxxx@xxxxxxxx', 'isModEnabled("xxxxxxxx")');
		//$result2=$extrafields->addExtraField('xxxxxxxx_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', -1, 0, '', '', 'xxxxxxxx@xxxxxxxx', 'isModEnabled("xxxxxxxx")');
		//$result3=$extrafields->addExtraField('xxxxxxxx_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', -1, 0, '', '', 'xxxxxxxx@xxxxxxxx', 'isModEnabled("xxxxxxxx")');
		//$result4=$extrafields->addExtraField('xxxxxxxx_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', -1, 0, '', '', 'xxxxxxxx@xxxxxxxx', 'isModEnabled("xxxxxxxx")');
		//$result5=$extrafields->addExtraField('xxxxxxxx_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', -1, 0, '', '', 'xxxxxxxx@xxxxxxxx', 'isModEnabled("xxxxxxxx")');

...
/**
 * \file        class/tuehederde.class.php
 * \ingroup     xxxxxxxx
 * \brief       This file is a CRUD class file for Tuehederde (Create/Read/Update/Delete)
 */
...
/**
 * Class for Tuehederde
 */
class Tuehederde extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'xxxxxxxx';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'tuehederde';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'xxxxxxxx_tuehederde';

	/**
	 * @var string 	If permission must be checkec with hasRight('xxxxxxxx', 'read') and not hasright('mymodyle', 'tuehederde', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'xxxxxxxx';

	/**
	 * @var string 	String with name of icon for tuehederde. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'tuehederde@xxxxxxxx' if picto is file 'img/object_tuehederde.png'.
	 */
	...
	//  */
	// public $table_element_line = 'xxxxxxxx_tuehederdeline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_tuehederde';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Tuehederdeline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Tuehederde', 'fk_element'=>'fk_tuehederde'));
	...
	//  */
	// protected $childtablesoncascade = array('xxxxxxxx_tuehederdedet');

	// /**
	//  * @var TuehederdeLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('xxxxxxxx', 'tuehederde', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('xxxxxxxx', 'tuehederde', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('xxxxxxxx', 'tuehederde_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'tuehederde/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'tuehederde/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'tuehederde/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'tuehederde/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->xxxxxxxx->dir_output.'/tuehederde/'.$oldref;
				$dirdest = $conf->xxxxxxxx->dir_output.'/tuehederde/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->xxxxxxxx->dir_output.'/tuehederde/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('xxxxxxxx','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('xxxxxxxx','xxxxxxxx_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'XXXXXXXX_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'XXXXXXXX_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'XXXXXXXX_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowTuehederde")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Tuehederde").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/xxxxxxxx/tuehederde_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowTuehederde");
	...
	public function getLinesArray()
	{
		...
		$objectline = new TuehederdeLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_tuehederde:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('XXXXXXXX_MYOBJECT_ADDON')) {
			$conf->global->XXXXXXXX_MYOBJECT_ADDON = 'mod_tuehederde_standard';
		}

		if (getDolGlobalString('XXXXXXXX_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('XXXXXXXX_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('XXXXXXXX_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/xxxxxxxx/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefTuehederde $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/xxxxxxxx/doc/";
...
/**
 * Class TuehederdeLine. You can also remove this and generate a CRUD class for lines objects.
 */
class TuehederdeLine extends CommonObjectLine
{
	// To complete with content of an object TuehederdeLine
	// We should have a field rowid, fk_tuehederde and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'tuehederde'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_tuehederde'

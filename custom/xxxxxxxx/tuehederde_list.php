...
/**
 *   	\file       tuehederde_list.php
 *		\ingroup    xxxxxxxx
 *		\brief      List page for tuehederde
 */
...
require_once __DIR__.'/class/tuehederde.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('xxxxxxxx', 'tuehederde', 'write');
	$permissiontodelete = $user->hasRight('xxxxxxxx', 'tuehederde', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("xxxxxxxx")) {
	accessforbidden('Module xxxxxxxx not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Tuehederde';
	$objectlabel = 'Tuehederde';
	$uploaddir = $conf->xxxxxxxx->dir_output;
...
$title = $langs->trans("Tuehederdes");
//$help_url = "EN:Module_Tuehederde|FR:Module_Tuehederde_FR|ES:Módulo_Tuehederde";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/xxxxxxxx/tuehederde_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-xxxxxxxx page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/xxxxxxxx/tuehederde_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendTuehederdeRef";
$modelmail = "tuehederde";
$objecttmp = new Tuehederde($db);

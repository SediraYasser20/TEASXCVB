...
/**
 *   	\file       receptiondeproduit_list.php
 *		\ingroup    productreceptions
 *		\brief      List page for receptiondeproduit
 */
...
require_once __DIR__.'/class/receptiondeproduit.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('productreceptions', 'receptiondeproduit', 'write');
	$permissiontodelete = $user->hasRight('productreceptions', 'receptiondeproduit', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("productreceptions")) {
	accessforbidden('Module productreceptions not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Receptiondeproduit';
	$objectlabel = 'Receptiondeproduit';
	$uploaddir = $conf->productreceptions->dir_output;
...
$title = $langs->trans("Receptiondeproduits");
//$help_url = "EN:Module_Receptiondeproduit|FR:Module_Receptiondeproduit_FR|ES:Módulo_Receptiondeproduit";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/productreceptions/receptiondeproduit_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-productreceptions page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/productreceptions/receptiondeproduit_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendReceptiondeproduitRef";
$modelmail = "receptiondeproduit";
$objecttmp = new Receptiondeproduit($db);

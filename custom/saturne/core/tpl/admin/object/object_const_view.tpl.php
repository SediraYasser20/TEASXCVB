<?php
// Initialize $constArray to avoid undefined variable warning
$constArray = []; // Fix: Ensure $constArray is always defined
...
if ($action == 'add_all_conf') {
    if (is_array($constArray) && !empty($constArray) && isset($constArray[$moduleNameLowerCase])) { // Fix: Check if $constArray[$moduleNameLowerCase] exists
        foreach ($constArray[$moduleNameLowerCase] as $const) {
            $code = $const['code'];
            if (empty($conf->global->$code) && (!isset($const['disabled']) || $const['disabled'] != 1)) { // Fix: Safely check 'disabled'
...
if ($action == 'delete_all_conf') {
    if (is_array($constArray) && !empty($constArray) && isset($constArray[$moduleNameLowerCase])) { // Fix: Check if $constArray[$moduleNameLowerCase] exists
        foreach ($constArray[$moduleNameLowerCase] as $const) {
            if (empty($conf->global->$const['code']) && (!isset($const['disabled']) || $const['disabled'] != 1)) { // Fix: Safely check 'disabled'
...
if (is_array($constArray) && !empty($constArray) && isset($constArray[$moduleNameLowerCase])) { // Fix: Check if $constArray[$moduleNameLowerCase] exists
    ...
    foreach ($constArray[$moduleNameLowerCase] as $const) {
        ...
        if ($user->admin) {
            print ajax_constantonoff(isset($const['disabled']) && $const['disabled'] == 1 ? '' : $const['code'], $input ?? [], $entity ?? null, $revertonoff ?? 0, $strict ?? 0, $forceReload ?? 0, $marginleftonlyshort ?? 2, $forcenoajax ?? 0, $setzeroinsteadofdel ?? 1, $suffix ?? '', $mode ?? ''); // Fix: Safely check 'disabled'
        }
        print '</td>';
        print '</tr>';
    }

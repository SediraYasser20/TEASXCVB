...
function saturne_banner_tab(object $object, string $paramId = 'ref', string $moreHtml = '', int $showNav = 1, string $fieldId = 'ref', string $fieldRef = 'ref', string $moreHtmlRef = '', bool $handlePhoto = false, array $moreParams = []): void
{
    ...
    $parameters = [];
    $customMoreHtmlRef = ''; // Fix: Initialize to avoid undefined variable
    $resHook = $hookmanager->executeHooks('saturneBannerTab', $parameters, $object);
    if ($resHook < 0) {
    ...
    if (!$handlePhoto) {
        $moreHtmlCondition = ($moreHtml !== 'none' && (!isset($moreParams['moreHtml']) || $moreParams['moreHtml'] !== 'none')) ? $moreHtml : '';
        dol_banner_tab($object, $paramId, $moreHtmlCondition, $showNav, $fieldId, $fieldRef, $saturneMoreHtmlRef, $moreParamsBannerTab);
    } else {
        global $conf, $form;
        ...
        $resHook = $hookmanager->executeHooks('saturneBannerTabCustomSubdir', $parameters, $object, $action);
        if ($resHook > 0) {
            if (!empty($hookmanager->resArray)) {
        ...
        print $form->showrefnav($object, $paramId, $moreHtmlCondition, $showNav, $fieldId, $fieldRef, $saturneMoreHtmlRef, $moreParamsBannerTab, 0, $moreHtmlLeft, $object->getLibStatut(6));
        print '</div>';
    }

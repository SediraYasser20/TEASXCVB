...
if (!empty($filter) && is_array($filter)) {
    foreach ($filter as $key => $value) {
        ...
        // Direct rowid comparison
        if ($key == 't.rowid') {
            $sqlwhere[] = $key . ' = ' . (int)$value;
        } 
        // Check if the field exists and is a date type
        elseif (isset($object->fields[$key]['type']) && 
                in_array($object->fields[$key]['type'], ['date', 'datetime', 'timestamp'])) {
            $sqlwhere[] = $key .' = \'' . $object->db->idate($value) . '\'';
        } 
        // Handle custom SQL (with additional safety)
        elseif ($key === 'customsql') {
            // Only add if value is a non-empty string and doesn't contain potentially harmful SQL
            if (is_string($value) && !empty($value) && 
                !preg_match('/(\b(DELETE|DROP|TRUNCATE|ALTER)\b)/i', $value)) {
                $sqlwhere[] = $value;
            }
        } 
        // Handle regular comparisons
            ...
            // Sanitize the value
            $sanitized_value = $object->db->escape($value);
            
            // Check if the value contains a wildcard
            if (strpos($sanitized_value, '%') === false) {
                // For exact matches or IN clause
                $sqlwhere[] = $key .' IN (\'' . $sanitized_value . '\')';
            } else {
                // For LIKE comparisons
                $sqlwhere[] = $key .' LIKE \'%' . $sanitized_value . '%\'';
            }
        }
    }
}

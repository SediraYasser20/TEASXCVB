  ...
  public function load_dashboard(): array
{
    ...
    $dashboardConfig = property_exists($user->conf, $confName) ? json_decode($user->conf->$confName) : (object) [
        'graphs' => (object) [
            'SurveysByFiscalYear' => (object) ['hide' => false]
        ]
    ];
...
public function getNbSurveysByMonth(): array
{
    ...
    // Initialize arrayNbSurveys to avoid undefined key errors
    $arrayNbSurveys = [
    ...
    for ($i = 1; $i < 13; $i++) {
        ...
        $array['data'][$arrayKey] = [
            $month,
            $arrayNbSurveys[0][$i] ?? 0,
            $arrayNbSurveys[1][$i] ?? 0,
            $arrayNbSurveys[2][$i] ?? 0
        ];
    }
    ksort($array['data']);

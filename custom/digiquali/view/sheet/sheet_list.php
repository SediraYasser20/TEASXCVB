...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'sheetlist'; // To manage different context of search
...
$hookmanager->initHooks(array('sheetlist')); // Note that conf->hooks_modules contains array
...
$searchAll = GETPOSTISSET('search_all') ? GETPOST('search_all', 'alphanohtml') : (GETPOSTISSET('sall') ? GETPOST('sall', 'alphanohtml') : ''); // Fix: Use GETPOSTISSET to avoid undefined key warning
$search     = [];
...
$arrayfields['t.nb_questions'] = ['label' => 'NbQuestions', 'checked' => 1, 'enabled' => 1, 'position' => 16]; // Note: Could add 'type' => 'integer' here to define type for nb_questions
foreach ($object->fields as $key => $val) {
    ...
    if (!empty($val['visible'])) {
        ...
        $arrayfields['t.' . $key] = [
            ...
            'help'     => isset($val['help']) ? $val['help'] : '' // Fix: Default to empty string if 'help' is not set
        ];
    }
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $searchAll && !$page) {
    ...
    $id  = $obj->rowid;
    header("Location: " . dol_buildpath('/digiquali/view/sheet/sheet_card.php', 1) . '?id=' . $id);
    exit;
}
...
foreach ($object->fields as $key => $val) {
    // Ensure $val is an array and 'type' key exists
    $val = is_array($val) ? $val : [];
    $type = $val['type'] ?? '';
    $cssforfield = (empty($val['css']) ? '' : $val['css']);

    if ($key == 'status' || $key == 'nb_questions') {
        $cssforfield .= ($cssforfield ? ' ' : '') . 'center';
    } elseif (in_array($type, ['date', 'datetime', 'timestamp'])) {
        $cssforfield .= ($cssforfield ? ' ' : '') . 'center';
    } elseif ($type === 'timestamp') {
        $cssforfield .= ($cssforfield ? ' ' : '') . 'nowrap';
    } elseif (in_array($type, ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && 
    ...
    if (!empty($arrayfields['t.' . $key]['checked'])) {
        ...
        // Added null coalescing and is_array checks
        $arrayofkeyval = $val['arrayofkeyval'] ?? [];
        $search_key = $search[$key] ?? '';

        if (is_array($arrayofkeyval) && !empty($arrayofkeyval)) {
            print $form->selectarray('search_' . $key, $arrayofkeyval, $search_key, 
                ($val['notnull'] ?? 0), 0, 0, '', 1, 0, 0, '', 
                (($key != 'status') ? 'maxwidth100' : 'maxwidth200'), 1);
        } elseif (strpos($type, 'integer:') === 0) {
            print $object->showInputField($val, $key, $search_key, '', '', 'search_', 'maxwidth125', 1);
        } elseif (!preg_match('/^(date|timestamp)/', $type)) {
            print '<input type="text" class="flat maxwidth75" name="search_' . $key . 
                  '" value="' . dol_escape_htmltag($search_key) . '">';
        }
        print '</td>';
    }
}
...
foreach ($object->fields as $key => $val) {
    ...
    if ($key == 'status' || $key == 'nb_questions') {
        ...
        $cssforfield .= ($cssforfield ? ' ' : '') . 'nowrap';
    } elseif (in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && $val['label'] != 'TechnicalID') {
        $cssforfield .= ($cssforfield ? ' ' : '') . 'right';
    }
    if (!empty($arrayfields['t.' . $key]['checked'])) {
...
// --------------------------------------------------------------------
$i          = 0;
$totalarray = ['nbfield' => 0]; // Fix: Initialize nbfield to 0 to avoid potential undefined key warning
while ($i < ($limit ? min($num, $limit) : $num)) {
    ...
    foreach ($object->fields as $key => $val) {
        $cssforfield = (empty($val['css']) ? '' : $val['css']);
        if (isset($val['type']) && in_array($val['type'], ['date', 'datetime', 'timestamp'])) { // Fix: Check if 'type' exists
            $cssforfield .= ($cssforfield ? ' ' : '') . 'center';
        } elseif ($key == 'status' || $key == 'nb_questions') {
            $cssforfield .= ($cssforfield ? ' ' : '') . 'center';
        }

        if (isset($val['type']) && in_array($val['type'], ['timestamp'])) { // Fix: Check if 'type' exists
            $cssforfield .= ($cssforfield ? ' ' : '') . 'nowrap';
        } elseif ($key == 'ref') {
            $cssforfield .= ($cssforfield ? ' ' : '') . 'nowrap';
        }

        if (isset($val['type']) && in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && !in_array($key, ['rowid', 'status'])) {
            $cssforfield .= ($cssforfield ? ' ' : '') . 'right';
        }

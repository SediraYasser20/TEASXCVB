...
$searchAll = GETPOSTISSET('search_all') ? GETPOST('search_all', 'alphanohtml') : (GETPOSTISSET('sall') ? GETPOST('sall', 'alphanohtml') : '');
$search     = [];
...
foreach ($search as $key => $val) {
    if ($key == 'status' && $val == 'specialCase') {
        $newStatus = [Question::STATUS_VALIDATED, Question::STATUS_LOCKED];
        if (!empty($newStatus)) {
...
foreach ($object->fields as $key => $val) {
    ...
    if (!empty($arrayfields['t.' . $key]['checked'])) {
        ...
        if (isset($val['arrayofkeyval']) && is_array($val['arrayofkeyval']) && !empty($val['arrayofkeyval'])) {
            print $form->selectarray('search_' . $key, $val['arrayofkeyval'], isset($search[$key]) ? $search[$key] : '', $val['notnull'], 0, 0, '', 1, 0, 0, '', (($key != 'status') ? 'maxwidth100' : 'maxwidth200'), 1);
...
// --------------------------------------------------------------------
$i          = 0;
$totalarray = ['nbfield' => 0]; // Fix: Initialize nbfield to 0 to avoid undefined key warning

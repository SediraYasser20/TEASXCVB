...
foreach ($object->fields as $key => $val) {
    if (!empty($val['searchall'])) $fieldstosearchall['t.' . $key] = $val['label']; // Fix: Check if searchall exists
}
...
foreach ($object->fields as $key => $val) {
    ...
    if (!empty($val['visible'])) {
        ...
        $arrayfields['t.' . $key] = [
            'label'    => $val['label'],
            'checked'  => (($visible < 0 || (isset($val['showinpwa']) && $val['showinpwa'] == 0 && $source == 'pwa')) ? 0 : 1), // Fix: Check if showinpwa exists
            'enabled'  => ($visible != 3 && dol_eval($val['enabled'], 1)),
            'position' => $val['position'],
            'help'     => isset($val['help']) ? $val['help'] : '', // Fix: Default to empty string if help is not set
            'css'      => isset($val['css']) ? $val['css'] : '' // Fix: Default to empty string if css is not set
        ];
    }
}

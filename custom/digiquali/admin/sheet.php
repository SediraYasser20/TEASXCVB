...
print '<td class="center">';
$categories = $tags->getListForItem(0, 'sheet'); // Fix: Check if sheet categories exist
$isCategoriesSet = property_exists($conf->global, 'DIGIQUALI_SHEET_MAIN_CATEGORIES_SET') && $conf->global->DIGIQUALI_SHEET_MAIN_CATEGORIES_SET; // Fix: Safely check constant
print (!empty($categories) || $isCategoriesSet) ? $langs->transnoentities('AlreadyGenerated') : $langs->transnoentities('NotCreated'); // Fix: Use categories or constant for status
print '</td><td class="center">';
print (!empty($categories) || $isCategoriesSet) ? '<a type="" class=" butActionRefused" value="">' . $langs->transnoentities('Create') . '</a>' : '<input type="submit" class="button" value="' . $langs->transnoentities('Create') . '">'; // Fix: Disable button if categories exist
print '</td><td class="center">';
...
print '<td class="center">';
$mainCategoryId = property_exists($conf->global, 'DIGIQUALI_SHEET_MAIN_CATEGORY') ? $conf->global->DIGIQUALI_SHEET_MAIN_CATEGORY : ''; // Fix: Safely access constant
print $formOther->select_categories('sheet', $mainCategoryId, 'main_category'); // Fix: Use safe category ID
print '</td><td class="center">';

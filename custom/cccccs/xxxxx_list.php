...
require_once __DIR__.'/class/xxxxx.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('cccccs', 'xxxxx', 'write');
	$permissiontodelete = $user->hasRight('cccccs', 'xxxxx', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("cccccs")) {
	accessforbidden('Module cccccs not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Xxxxx';
	$objectlabel = 'Xxxxx';
	$uploaddir = $conf->cccccs->dir_output;
...
$title = $langs->trans("Xxxxxs");
//$help_url = "EN:Module_Xxxxx|FR:Module_Xxxxx_FR|ES:Módulo_Xxxxx";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/cccccs/xxxxx_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-cccccs page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/cccccs/xxxxx_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendXxxxxRef";
$modelmail = "xxxxx";
$objecttmp = new Xxxxx($db);

...
/**
 * \file        class/xxxxx.class.php
 * \ingroup     cccccs
 * \brief       This file is a CRUD class file for Xxxxx (Create/Read/Update/Delete)
 */
 ...
 * Class for Xxxxx
 */
class Xxxxx extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'cccccs';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'xxxxx';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'cccccs_xxxxx';

	/**
	 * @var string 	If permission must be checkec with hasRight('cccccs', 'read') and not hasright('mymodyle', 'xxxxx', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'cccccs';

	/**
	 * @var string 	String with name of icon for xxxxx. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'xxxxx@cccccs' if picto is file 'img/object_xxxxx.png'.
	 */
	...
	//  */
	// public $table_element_line = 'cccccs_xxxxxline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_xxxxx';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Xxxxxline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Xxxxx', 'fk_element'=>'fk_xxxxx'));
	...
	//  */
	// protected $childtablesoncascade = array('cccccs_xxxxxdet');

	// /**
	//  * @var XxxxxLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('cccccs', 'xxxxx', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('cccccs', 'xxxxx', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('cccccs', 'xxxxx_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'xxxxx/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'xxxxx/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'xxxxx/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'xxxxx/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->cccccs->dir_output.'/xxxxx/'.$oldref;
				$dirdest = $conf->cccccs->dir_output.'/xxxxx/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->cccccs->dir_output.'/xxxxx/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('cccccs','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('cccccs','cccccs_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'CCCCCS_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'CCCCCS_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'CCCCCS_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowXxxxx")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Xxxxx").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/cccccs/xxxxx_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new XxxxxLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_xxxxx:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('CCCCCS_MYOBJECT_ADDON')) {
			$conf->global->CCCCCS_MYOBJECT_ADDON = 'mod_xxxxx_standard';
		}

		if (getDolGlobalString('CCCCCS_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('CCCCCS_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('CCCCCS_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/cccccs/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefXxxxx $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/cccccs/doc/";
...
/**
 * Class XxxxxLine. You can also remove this and generate a CRUD class for lines objects.
 */
class XxxxxLine extends CommonObjectLine
{
	// To complete with content of an object XxxxxLine
	// We should have a field rowid, fk_xxxxx and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'xxxxx'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_xxxxx'

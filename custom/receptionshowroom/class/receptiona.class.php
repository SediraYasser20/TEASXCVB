...
/**
 * \file        class/receptiona.class.php
 * \ingroup     receptionshowroom
 * \brief       This file is a CRUD class file for RECEPTIONa (Create/Read/Update/Delete)
 */
...
/**
 * Class for RECEPTIONa
 */
class RECEPTIONa extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'receptionshowroom';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'receptiona';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'receptionshowroom_receptiona';

	/**
	 * @var string 	If permission must be checkec with hasRight('receptionshowroom', 'read') and not hasright('mymodyle', 'receptiona', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'receptionshowroom';

	/**
	 * @var string 	String with name of icon for receptiona. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'receptiona@receptionshowroom' if picto is file 'img/object_receptiona.png'.
	 */
	...
	public $fields = array(
		...
		"serialnumber" => array("type" => "varchar(128)", "label" => "Serial Number", "enabled" => "1", 'position' => 0, 'notnull' => 0, "visible" => "1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'receptionshowroom_receptionaline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_receptiona';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'RECEPTIONaline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'RECEPTIONa', 'fk_element'=>'fk_receptiona'));
	...
	//  */
	// protected $childtablesoncascade = array('receptionshowroom_receptionadet');

	// /**
	//  * @var RECEPTIONaLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('receptionshowroom', 'receptiona', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('receptionshowroom', 'receptiona', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('receptionshowroom', 'receptiona_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'receptiona/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'receptiona/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'receptiona/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'receptiona/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->receptionshowroom->dir_output.'/receptiona/'.$oldref;
				$dirdest = $conf->receptionshowroom->dir_output.'/receptiona/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->receptionshowroom->dir_output.'/receptiona/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('receptionshowroom','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('receptionshowroom','receptionshowroom_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'RECEPTIONSHOWROOM_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'RECEPTIONSHOWROOM_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'RECEPTIONSHOWROOM_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowRECEPTIONa")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("RECEPTIONa").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/receptionshowroom/receptiona_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowRECEPTIONa");
	...
	public function getLinesArray()
	{
		...
		$objectline = new RECEPTIONaLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_receptiona:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('RECEPTIONSHOWROOM_MYOBJECT_ADDON')) {
			$conf->global->RECEPTIONSHOWROOM_MYOBJECT_ADDON = 'mod_receptiona_standard';
		}

		if (getDolGlobalString('RECEPTIONSHOWROOM_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('RECEPTIONSHOWROOM_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('RECEPTIONSHOWROOM_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/receptionshowroom/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefRECEPTIONa $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/receptionshowroom/doc/";
...
/**
 * Class RECEPTIONaLine. You can also remove this and generate a CRUD class for lines objects.
 */
class RECEPTIONaLine extends CommonObjectLine
{
	// To complete with content of an object RECEPTIONaLine
	// We should have a field rowid, fk_receptiona and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'receptiona'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_receptiona'

...
/**
 *   	\file       receptiona_list.php
 *		\ingroup    receptionshowroom
 *		\brief      List page for receptiona
 */
...
require_once __DIR__.'/class/receptiona.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('receptionshowroom', 'receptiona', 'write');
	$permissiontodelete = $user->hasRight('receptionshowroom', 'receptiona', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("receptionshowroom")) {
	accessforbidden('Module receptionshowroom not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'RECEPTIONa';
	$objectlabel = 'RECEPTIONa';
	$uploaddir = $conf->receptionshowroom->dir_output;
...
$title = $langs->trans("RECEPTIONas");
//$help_url = "EN:Module_RECEPTIONa|FR:Module_RECEPTIONa_FR|ES:Módulo_RECEPTIONa";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/receptionshowroom/receptiona_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-receptionshowroom page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/receptionshowroom/receptiona_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendRECEPTIONaRef";
$modelmail = "receptiona";
$objecttmp = new RECEPTIONa($db);

...
if (empty($reshook)) {
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		// $object->category = GETPOST('category', 'alpha'); // Handled by passing to create method
		// $object->service = GETPOST('service', 'alpha');   // Handled by passing to create method
		// $object->resource = GETPOST('resource', 'alpha'); // Handled by passing to create method
		...
		if (!$error) {
			...
			$category_custom_val = GETPOST('category', 'alpha');
			$service_custom_val = GETPOST('service', 'alpha');
			$resource_custom_val = GETPOST('resource', 'alpha');
			$ret = $object->create($user, $category_custom_val, $service_custom_val, $resource_custom_val);
			if ($ret > 0) {
				$db->commit();
...
if ($action == 'create') {
	...
	$category_options = array(
		'' => $langs->trans("SelectOne"), // Add a default empty option
		'Payment Service' => 'Payment Service',
		'Moyenne generaux' => 'Moyenne generaux',
		'Les Pertes / Errors' => 'Les Pertes / Errors',
		'Consommation' => 'Consommation',
		'Prime Exceptionnel' => 'Prime Exceptionnel',
		'Pub' => 'Pub',
		'Socials' => 'Socials',
		'Autre' => 'Autre'
	);
	print $form->selectarray('category', $category_options, GETPOST('category', 'alpha'), 0, 0, 0, '', 0, 0, 0, '', 'minwidth300', 1);
	print '</td></tr>';

	// Service Dropdown
	print '<tr><td>'.$langs->trans("Service").'</td><td>';
	$service_options = array('' => $langs->trans("SelectOne"));
	if (isset($db)) { // Ensure $db is available
		$sql_service = "SELECT param FROM ".MAIN_DB_PREFIX."extrafields WHERE rowid = 38";
		dol_syslog("Service Dropdown: SQL query: " . $sql_service, LOG_DEBUG);
		$res_service = $db->query($sql_service);
		if ($res_service) {
			$obj_service = $db->fetch_object($res_service);
			if ($obj_service && !empty($obj_service->param)) {
				dol_syslog("Service Dropdown: Raw param string from DB: " . $obj_service->param, LOG_DEBUG);
				// Attempt to unserialize, with error suppression for warnings if param is malformed.
				$params_array = @unserialize($obj_service->param);
				dol_syslog("Service Dropdown: Unserialized params_array: " . print_r($params_array, true), LOG_DEBUG);

				if ($params_array !== false && isset($params_array['options']) && is_array($params_array['options'])) {
					foreach ($params_array['options'] as $key => $value) {
						$service_options[$key] = $value;
					}
				} else {
					dol_syslog("Service Dropdown: Failed to unserialize or 'options' key not found/not an array. Param was: " . $obj_service->param, LOG_WARNING);
				}
				dol_syslog("Service Dropdown: Final service_options array: " . print_r($service_options, true), LOG_DEBUG);
			} else {
				dol_syslog("Service Dropdown: No object or empty param found for rowid 38 when fetching from llx_extrafields.", LOG_WARNING);
			}
			$db->free($res_service); // Free the result set
		} else {
			dol_syslog("Service Dropdown: DB query failed for llx_extrafields: " . $db->lasterror(), LOG_ERR);
		}
	} else {
		dol_syslog("Service Dropdown: \$db object not available.", LOG_ERR);
	}
	print $form->selectarray('service', $service_options, GETPOST('service', 'alpha'), 0, 0, 0, '', 0, 0, 0, '', 'minwidth300', 1);
	print '</td></tr>';

	// Resource Dropdown
	print '<tr><td>'.$langs->trans("Resource").'</td><td>';
	$resource_options = array('' => $langs->trans("SelectOne"));
	if (isset($db)) { // Ensure $db is available
		$sql_resource = "SELECT rowid, ref FROM ".MAIN_DB_PREFIX."resource WHERE active = 1 ORDER BY ref ASC";
		dol_syslog("Resource Dropdown: SQL query: " . $sql_resource, LOG_DEBUG);
		$res_resource = $db->query($sql_resource);
		if ($res_resource) {
			$num_resources = $db->num_rows($res_resource);
			dol_syslog("Resource Dropdown: Number of active resources found: " . $num_resources, LOG_DEBUG);
			if ($num_resources > 0) {
				while ($obj_resource = $db->fetch_object($res_resource)) {
					$resource_options[$obj_resource->rowid] = $obj_resource->ref;
				}
			} else {
				dol_syslog("Resource Dropdown: No active resources found in llx_resource.", LOG_WARNING);
			}
			dol_syslog("Resource Dropdown: Final resource_options array: " . print_r($resource_options, true), LOG_DEBUG);
			$db->free($res_resource); // Free the result set
		} else {
			dol_syslog("Resource Dropdown: DB query failed for llx_resource: " . $db->lasterror(), LOG_ERR);
		}
	} else {
		dol_syslog("Resource Dropdown: \$db object not available.", LOG_ERR);
	}
	print $form->selectarray('resource', $resource_options, GETPOST('resource', 'alpha'), 0, 0, 0, '', 0, 0, 0, '', 'minwidth300', 1);
	print '</td></tr>';
	...
	if (isModEnabled('bank')) {
		...
		if ($object->fk_bank > 0) {
			...
			if ($result <= 0) {
				...
				// Category
				print '<tr><td class="titlefield">'.$langs->trans("Category").'</td><td colspan="3">'; // colspan="3" if previous td for BankTransactionLine has it
				if (property_exists($bankline, 'category_custom')) {
					print $bankline->category_custom;
				}
				print '</td></tr>';

				// Service
				print '<tr><td class="titlefield">'.$langs->trans("Service").'</td><td colspan="3">';
				if (property_exists($bankline, 'service_custom') && !empty($bankline->service_custom)) {
					$service_display_value = $bankline->service_custom; // Default to key if lookup fails
					$sql_service_lookup = "SELECT param FROM ".MAIN_DB_PREFIX."extrafields WHERE rowid = 38";
					$res_service_lookup = $db->query($sql_service_lookup);
					if ($res_service_lookup) {
						$obj_service_lookup = $db->fetch_object($res_service_lookup);
						if ($obj_service_lookup && !empty($obj_service_lookup->param)) {
							$params_array_lookup = unserialize($obj_service_lookup->param);
							if (isset($params_array_lookup['options']) && is_array($params_array_lookup['options']) && isset($params_array_lookup['options'][$bankline->service_custom])) {
								$service_display_value = $params_array_lookup['options'][$bankline->service_custom];
							}
						}
					}
					print $service_display_value;
				}
				print '</td></tr>';

				// Resource
				print '<tr><td class="titlefield">'.$langs->trans("Resource").'</td><td colspan="3">';
				if (property_exists($bankline, 'resource_custom') && !empty($bankline->resource_custom)) {
					$resource_display_value = $bankline->resource_custom; // Default to key if lookup fails
					$sql_resource_lookup = "SELECT ref FROM ".MAIN_DB_PREFIX."resource WHERE rowid = ".((int) $bankline->resource_custom);
					$res_resource_lookup = $db->query($sql_resource_lookup);
					if ($res_resource_lookup) {
						$obj_resource_lookup = $db->fetch_object($res_resource_lookup);
						if ($obj_resource_lookup && !empty($obj_resource_lookup->ref)) {
							$resource_display_value = $obj_resource_lookup->ref;
						}
					}
					print $resource_display_value;
				}
				print '</td></tr>';
			}
		} else {
